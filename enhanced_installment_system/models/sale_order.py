...
class SaleOrder(models.Model):
    ...
    def _compute_has_installments(self):
        for order in self:
            order.has_installments = bool(order.installment_schedule_id)
    ...
    def action_generate_installment_schedule(self):
        ...
        if self.state not in ['draft', 'sent']:
            raise UserError(_("Installment schedule can only be generated for draft or sent orders"))
        
        if self.installment_schedule_id:
            raise UserError(_("This order already has an installment schedule"))
    ...
    def action_view_installment_schedule(self):
        ...
        if not self.installment_schedule_id:
            raise UserError(_("No installment schedule found for this order"))
